...
(function () {
  ...
  var ModelFormController = class extends Stimulus.Controller {
    static targets = ["rows", "template", "preview", "optionHeader"];
    ...
    // Keep a row's "→ table" suggestion in step with its column name as it's typed.
    nameInput(event) {
      var row = event.target.closest("[data-row]");
      if (row) this.updateSuggestion(row);
    }
    ...
    syncRow(row) {
      ...
      this.updateSuggestion(row);
    }

    // The picker's default option mirrors Rails' inference: column "zebra"
    // suggests table "zebras". Approximate English pluralization — the live
    // preview shows the exact result.
    updateSuggestion(row) {
      var option = row.querySelector("[data-ref-suggestion]");
      if (!option) return;
      var field = row.querySelector('input[name="attributes[][name]"]');
      var name = field ? field.value.trim() : "";
      option.textContent = name ? "→ " + this.pluralize(name) : "→ matches name";
    }
